...
show_help() {
  ...
  echo "  -s, --slim              Build slim image instead of full image"
...
# Execute the build
docker buildx build --builder "$BUILDER" --platform linux/amd64,linux/arm64 -t "$FULL_IMAGE_NAME" --build-context plugins="$PLUGINSDIR" $PACKAGES --push --provenance=false "$BASEDIR"
